...
@section('title', 'Edit Data Log Aktivitas')
	...
	<div class="col-12 grid-margin stretch-card">
		<div class="card">
			<div class="card-body">
				...
				<form class="forms-sample"  action="{{ route('log.update', $logs->id) }}" method="POST">
					{{ method_field('PUT') }}
					{{ csrf_field() }}
					<div class="form-group">
						...
						<select class="form-control" name="car_id" id="car_id">
							...
							@foreach($cars as $car)
								<option <?= ($logs->car_id == $car->id) ? 'selected' : '' ; ?> value="{{ $car->id }}">{{ $car->id }} ( {{$car->car_plat_number}} )</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						...
						<select class="form-control" name="owner_id" id="owner_id">
							...
							@foreach($owners as $owner)
								<option <?= ($logs->owner_id == $owner->id) ? 'selected' : '' ; ?>  value="{{ $owner->id }}">{{$owner->owner_name}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						...
						<select class="form-control" name="driver_id" id="driver_id">
							...
							@if($logs->driver_id == null)

							@else
								@foreach($drivers as $driver)
									<option  <?= ($logs->driver_id == $driver->id) ? 'selected' : '' ; ?>  value="{{ $driver->id }}">{{$driver->driver_name}}</option>
								@endforeach
							@endif
